...
namespace Ahoy.Classes
{
  ...
  class Links 
  {
    ...
    {
      StreamReader linksWatcher = new StreamReader(@Environment.CurrentDirectory + @"\links.txt", Encoding.UTF8); // Открываем файл с ссылками для чтения      
    ...
    // Возвращаем словарь
    public Dictionary<string, List<string>> SortedLinks
    {
      get { return sortedLinks; }
    }
